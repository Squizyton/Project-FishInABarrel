...
public class SceneReferencePropertyDrawer : PropertyDrawer
{
    ...
    private void DrawSceneInfoGUI(Rect position, BuildUtils.BuildScene buildScene, int sceneControlID)
    {
        ...
        {
            iconContent = EditorGUIUtility.IconContent("d_Invalid");
        ...
        {
            iconContent = EditorGUIUtility.IconContent("d_GreenCheckmark");
        ...
        {
            iconContent = EditorGUIUtility.IconContent("d_console.warnicon.sml");
            labelContent.text = "BuildIndex: " + buildScene.buildIndex;
        ...
        using (new EditorGUI.DisabledScope(readOnly))
        {
            ...
            else
            {
                ...
                buttonRect.x += buttonRect.width;
                var titleString = "Open In Editor";

                if (buildScene.buildIndex != -1)
                {
                    if (!EditorSceneManager.GetSceneByPath(buildScene.assetPath).isLoaded)
                    {
                        if (DrawUtils.ButtonHelper(buttonRect, titleString, titleString, EditorStyles.miniButtonRight,
                                "Open Scene Additively"))
                        {
                            BuildUtils.OpenSceneInEditor(buildScene);
                        }
                    }
                    else
                    {
                        if (DrawUtils.ButtonHelper(buttonRect, "Close Scene", "Close Scene", EditorStyles.miniButtonRight,
                                "Open Scene Additively"))
                        {
                            BuildUtils.CloseSceneInEditor(buildScene);
                        }
                    }
                }
            }
    ...
    private static class BuildUtils
    {
        ...
        /// <summary>
        /// Opens the given scene in the editor
        /// </summary>
        /// <param name="buildScene"></param>
        public static void OpenSceneInEditor(BuildScene buildScene)
        {
            EditorSceneManager.OpenScene(buildScene.assetPath, OpenSceneMode.Additive);
        }
        
        /// <summary>
        /// Close's the scene in the editor
        /// </summary>
        /// <param name="buildScene"></param>
        public static void CloseSceneInEditor(BuildScene buildScene)
        {

            EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo();
            
            EditorSceneManager.CloseScene(EditorSceneManager.GetSceneByPath(buildScene.assetPath), true);
        }
